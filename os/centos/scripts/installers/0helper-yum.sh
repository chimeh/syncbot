...
if runon_aliyun;then
  ...
  sed -i 's|^#?\baseurl=https\?://mirror.centos.org/centos/$releasever|baseurl=http://mirrors.cloud.aliyuncs.com/centos/$releasever|g' /etc/yum.repos.d/*.repo
  yum install -y --nogpgcheck  epel-release
  sed -i 's|^#?baseurl=https://download.fedoraproject.org/pub/epel|baseurl=https://mirrors.aliyun.com/epel|g' /etc/yum.repos.d/epel*
  sed -i 's|^metalink|#metalink|' /etc/yum.repos.d/epel*
  exit 0
fi
